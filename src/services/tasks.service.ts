...
export class TasksService {
	...
	async update(projectId: string, taskId: string, task: Partial<Task>): Promise<Task> {
		await this.tasksRepository.update({ id: taskId, projectId }, task);
		return this.tasksRepository.findOne({ where: { id: taskId, projectId } });
	}

	async delete(projectId: string, taskId: string): Promise<void> {
		await this.tasksRepository.delete({ id: taskId, projectId });
	}
